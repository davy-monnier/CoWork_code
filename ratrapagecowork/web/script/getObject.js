...
function redirectPost(url, data) {
    ...
    function show(){
        ...
        if(time1 > time2) alert("erreur la date de fin doit être supèrieure à la date de début.");
         var rep = $.post("[URL_API]/routes/getObject.php",{id_type : id_t ,id_site :$("#site").text() , datedebut: time1.replace("/","-") , datefin: time2.replace("/","-")},function( data ) {
            			
              				
            				var objs = JSON.parse(data);              
            				var toinsert ="";               
                			for( var obj in objs){									
                           			toinsert += showCards(objs[obj].nom, objs[obj].description ,"./image/"+objs[obj].image ,"reserver",objs[obj].id,time1,time2,objs[obj].quantite);                       			                                       	
	           				}
	...
	function reserve(id_c , id_o ,dt_d , dt_f){
     		$.post("[URL_API]/routes/newReservation.php", {id_client: id_c , id_object: id_o, date_debut: dt_d ,date_fin: dt_f},function(data){
               if(data == "ok")alert("Reservation enregistrée.");
               else alert("Reservation Impossible."+data);
              redirectPost("[URL_API]/web/mesService.php","");
            });
    }